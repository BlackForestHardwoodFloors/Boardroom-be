...
export const handler = requestHandler(async (event) => {
  ...
  try {
    const site = new Site();
    site.id = parseInt(requestBody.id);
    ...
    site.modifiedBy = parseInt(requestBody.modifiedBy);
    site.modifiedTime = new Date(requestBody.modifiedTime);

    await upsertSite(site);
    return {
      statusCode: 200,
      body: "Site Updated successfully",
    };
  } catch (error: any) {
